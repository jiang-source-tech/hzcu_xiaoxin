...
- 当前绝对硬边界包括危机场景、官方/私人联系方式代取、竞赛私有资源索取，以及心理咨询代预约等；这些场景不交给 LLM 路由自由发挥，避免事实错配或现实代办越界。
...
- 用户要求“小芯直接帮我预约心理咨询”时，路由应稳定进入 `hard_template`，回复只能使用已有知识库渠道 `88296000` 和 `理四114`，不能混入校医务室等其他校园地点事实。
...
- 心理咨询代预约必须稳定拒绝代办，并给出 `88296000` / `理四114`；不得出现“体育馆附近”等校医务室事实错配。

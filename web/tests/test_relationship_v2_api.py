...
class RelationshipV2ApiTest(unittest.TestCase):
    ...
    def test_v2_scenes_endpoint_is_disabled(self):
        response = self.client.get("/api/v2/relationship-selfplay/scenes")

        self.assertEqual(response.status_code, 410)
        self.assertIn("已下线", response.get_json()["error"])

    def test_v2_run_endpoint_is_disabled_before_streaming(self):
        with patch.object(app_module.scene_runner_v2, "run_scene_streaming") as run_streaming:
            response = self.client.post(
                "/api/v2/relationship-selfplay/run",
                json={"scene": "anxious_prospective"},
            )
